...
class NewServerView(QWidget):
    ...
    def _start_server(self):
        ...
        try:
            if self.active_runner.start_worker is not None:
                self.active_runner.start_worker.log_message.connect(self._on_server_log)
                # The preflight ``progress`` signal is a plain text
                # line ("[LocalServer] ...") — feed it into the same
                # in-launcher console so the user sees the server's
                # startup pipeline.
